<div class="container">
    ...
    border-bottom-right-radius: 50px;">
                <?= form_open_multipart('kelas/buat'); ?>
                <label for="">Pasang sampul kelas:</label>
                <input type="file" class="form-control rounded-pill" name="sampul" value="value=" <?= set_value('sampul') ?>"">
                ...
                <div class="row d-flex flex-lg-column justify-content-between">
                    <div class="col-auto">
                        <div class="mb-3">
                            <select class="form-select rounded-pill kelas" name="kelas">
                                ...
                                <?php foreach ($matkul as $row) : ?>
                                    <option data-id="<?= $row['kode_matkul'] ?>" value="<?= $row['matkul'] ?>"><?= $row['matkul'] ?>(<?= $row['kode_matkul'] ?>)</option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="form-floating mb-3">
                            <textarea value="<?= set_value('deskripsi') ?>" class="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 100px" name="deskripsi"></textarea>
                    ...
                    <div class="col-auto d-flex flex-column mb-3">
                        ...
                        <input class="form-control rounded-pill" type="date" name="tanggal" id="date" value="<?= set_value('tanggal') ?>">
                    ...
                    <div class="row mb-3">
                        <div class="col-6 d-flex flex-column mb-3">
                            ...
                            <input class="form-control rounded-pill" type="time" name="mulai" id="mulai" value="<?= set_value('mulai') ?>">
                            <?= form_error('mulai', '<small class="text-danger ms-3">', '</small>')  ?>

                        </div>
                        <div class="col-6 d-flex flex-column mb-3">
                            <label for="selesai">Waktu selesai :</label>
                            <input class="form-control rounded-pill" type="time" name="selesai" id="selesai" value="<?= set_value('selesai') ?>">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6 d-flex flex-column mb-3">
                            ...
                            <input placeholder="ex: https://meet.google.com/xak-nktb-txs" class="form-control rounded-pill" type="text" name="link" id="link">
                            <?= form_error('link', '<small class="text-danger ms-3">', '</small>')  ?>
                        </div>
                    </div>
                </div>
            </div>
            <button type="submit" class="fs-5 fw-700 btn gabung btn-primary" style="width : 100% ; height: 75px;text-decoration : none; color : white">Buat Kelas</button>
